...
const PRODUCTS_DATA = [
  {
    ...
    category: "gear",
    shortDescription:
      "پکیج پرتابل و مستقل از برق پمپ دنده‌ای صنعتی جابه‌جایی مثبت ۴ اینچ با کاور سایلنت، موتور دیزل و دبی ۴۲ مترمکعب در ساعت؛ طراحی‌شده برای انتقال فرآورده‌های نفتی، روغن‌های سنگین، سوخت، لجن هیدروکربنی و سیالات غلیظ.",
...
async function seed() {
  ...
  for (const prod of PRODUCTS_DATA) {
    ...
    if (existing.docs.length > 0) {
      console.log(`Updating existing product: ${prod.title}`);
      ...
      console.log(`Creating new product: ${prod.title}`);
